...
KeyboardWatcher.prototype._handleKeyDown = function(event) {
    this._setKeyBool(true, event.which);
};

KeyboardWatcher.prototype._handleKeyUp = function(event) {
    this._setKeyBool(false, event.which);
};

KeyboardWatcher.prototype._setKeyBool = function (down, code) {
...
KeyboardWatcher.prototype.onExitRequested = function (handler) {

    $(this).bind("exitRequested", handler);
};

//  Bind an event handler to the "actionPressed" event
KeyboardWatcher.prototype.onActionPressed = function (handler) {

    $(this).bind("actionPressed", handler);
};

KeyboardWatcher.prototype._debugGame = function () {
    if (window.debugGame) {
